...
export async function updateFutureWorkouts(
    supabase: SupabaseClient, 
    mesoId: string, 
    templates: Record<string, { remainingWorkouts: any[], exercises: any[] }>
) {
    // 1. Find all future incomplete workouts
    const { data: workouts } = await supabase
        .from('workouts')
    ...
    // 2. Delete ALL future exercises so we can cleanly replace them
    const allTargetIds = workouts.map(w => w.id);
    await supabase.from('workout_exercises').delete().in('workout_id', allTargetIds);

    const payloads: any[] = [];
    
    // 3. Rebuild exactly what the user laid out
    for (const [typeName, template] of Object.entries(templates)) {
        
        // Loop over the remaining workouts for this specific template type
        template.remainingWorkouts.forEach((rw: any, wIdx: number) => {
            
            // Safety check: ensure this workout id is actually in our incomplete list
            if (allTargetIds.includes(rw.id)) {
                
                template.exercises.forEach((ex: any, i: number) => {
                    // Pull the exact sets the user typed in for THIS specific week
                    const targetSets = ex.manualSets[wIdx] || 3; 
                    
                    const setResults = Array(targetSets).fill({ 
                        weight: null, reps: null, 
                        dropsets: ex.isDropset ? [{ weight: null, reps: null }] : [] 
                    });
