...
<script lang="ts">
  ...
  // Template manipulation logic updated for new data structure
  function addExercise(typeName: string) {
    const template = planTemplates[typeName];
    template.exercises = [...template.exercises, { 
        name: "", 
        isDropset: false, 
        manualSets: Array(template.remainingWorkouts.length).fill(3) 
    }];
    planTemplates = planTemplates;
  }

  function removeExercise(typeName: string, index: number) {
    planTemplates[typeName].exercises.splice(index, 1);
    planTemplates = planTemplates;
  }

  function moveExercise(typeName: string, index: number, direction: 'up' | 'down') {
    const list = planTemplates[typeName].exercises;
    const newIndex = direction === 'up' ? index - 1 : index + 1;
    if (newIndex < 0 || newIndex >= list.length) return;
  ...
  function selectExercise(typeName: string, exIndex: number, name: string) {
    planTemplates[typeName].exercises[exIndex].name = name;
    activeSearch = null;
  }
</script>
...
<Modal widthClass="w-full max-w-3xl" on:close={() => dispatch('close')}>
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-400 mt-1">Adjust sets for your remaining workouts.</p>
        </div>
    </div>

    <div class="flex-1 overflow-y-auto space-y-8 pr-2 custom-scrollbar">
        {#each Object.entries(planTemplates) as [typeName, templateData]}
            <div class="bg-gray-800 border border-gray-700 rounded-xl overflow-visible relative">
                <div class="bg-gray-700/50 p-3 border-b border-gray-700 flex justify-between items-center">
                    <h3 class="font-bold text-white text-sm">{typeName}</h3>
                    <div class="grid grid-cols-[40px_40px_20px] gap-2 items-center text-center text-[10px] font-bold uppercase text-gray-400 tracking-wider">
                        <span>Drop</span> <span></span><span></span>
                    </div>
                </div>
                ...
                <div class="p-3 space-y-2">
                    ...
                    {#each templateData.exercises as ex, exIdx}
                        <div class="grid grid-cols-[1fr_40px_40px_20px] gap-2 items-center">
                            <div class="relative w-full">
                                <input 
                                    type="text" 
                                    placeholder="Search..." 
                            ...
                            <div class="flex flex-col gap-1">
                                <button on:click={() => moveExercise(typeName, exIdx, 'up')} disabled={exIdx === 0} class="text-gray-500 hover:text-white disabled:opacity-30 disabled:hover:text-gray-500"><ArrowUp size={14} /></button>
                                <button on:click={() => moveExercise(typeName, exIdx, 'down')} disabled={exIdx === templateData.exercises.length - 1} class="text-gray-500 hover:text-white disabled:opacity-30 disabled:hover:text-gray-500"><ArrowDown size={14} /></button>
                            </div>
                        ...
                        <div class="pl-[10px] pr-[90px] mt-1 mb-6">
                            <div class="bg-gray-900/50 rounded-lg p-2 border border-gray-700/50 flex gap-2 overflow-x-auto custom-scrollbar">
                                {#each templateData.remainingWorkouts as rw, wIdx}
                                    <div class="flex flex-col items-center flex-1 min-w-[40px]">
                                        <span class="text-[10px] font-bold text-gray-500 mb-1">W{rw.week}</span>
